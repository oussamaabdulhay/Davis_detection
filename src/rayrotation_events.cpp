...
Vector3D<float> rayrotation_events::Update_unit_vector(MatrixXd rotated_matrix)
{
    ...
    t_results.z = U_v.x * rotated_matrix(2, 0) + U_v.y * rotated_matrix(2, 1) + U_v.z * rotated_matrix(2, 2);
     ...
     obj_pos.x=0;
     obj_pos.y =-1*t_results.y*100;;// -t_results.y*100;// It is better to flip this from origin //Adjusted the code to the time stamp of the message.
...
{
    float theta_yaw = -1*(0.7844 / 346.0) * ball_location.x;
    float theta_roll = (0.6041/ 260.0) * ball_location.y;
